...
Agile collaboration rule: a milestone is a durable goal view, not a release or sprint version. New requirements append work items or phases; they do not force a new milestone version. Use stable phase/spec IDs, workstream/team ownership, dependency edges, write sets, leases, source snapshots and decision records to coordinate parallel teams. `milestone.toml.revision` is an internal CAS epoch only; never ask a team to choose or increment it. Product/npm/release versions are managed separately when an external release is actually made.
