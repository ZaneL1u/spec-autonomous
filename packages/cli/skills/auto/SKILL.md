...
`auto` always checks `discussion.next` before a phase. If cards are empty, show the persisted decision summary and continue immediately. If cards marked `requires_user` exist, pause with recommended and alternative buttons; after `discussion.apply`, resume the same run.
