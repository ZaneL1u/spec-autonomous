...
import { createProviderManager } from '../lib/providers.mts';
import { errorMessage } from '../lib/errors.mts';
try {
  ...
  const child = spawn(command[0], [...command.slice(1), ...process.argv.slice(2)], { env: manager.env, stdio: 'inherit', shell: false });
