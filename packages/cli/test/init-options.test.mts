...
import { resolveInitOptions, type InitOptions, type ResolveInitIo, type SelectConfig } from '../lib/init-options.mjs';
import type { DetectionReport } from '../lib/provider-cli.mjs';
function fixture(t: TestContext, report: Partial<DetectionReport> = {}) {
 const root = mkdtempSync(join(tmpdir(), 'sa init options ')); t.after(() => rmSync(root, { recursive: true, force: true }));
 const io: ResolveInitIo = { detection: async () => ({ root, detected: [], selected: null, warnings: [], ...report }), env: {}, input: {isTTY:true}, output:{isTTY:true} };
 return { root, io };
}
test('missing values are prompted once and explicit values survive', async t => {
 const {io} = fixture(t); const questions: SelectConfig<any>[] =[];
...
test('detected provider and host are reused and CI/structured modes never prompt',async t=>{
 ...
 for(const [options,extra] of [[{nonInteractive:true},{}],[{json:true},{}],[{format:'toml'},{}],[{}, {env:{CI:'1'}}],[{}, {input:{isTTY:false}}]] as [InitOptions, Partial<ResolveInitIo>][]) await assert.rejects(resolveInitOptions(options,{...empty.io,...extra,select:()=>assert.fail()}),/provider_selection_required/);
