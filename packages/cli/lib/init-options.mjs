...
export async function selectInitOption(config, { input = process.stdin, output = process.stderr } = {}) {
  const { select } = await import('@inquirer/prompts');
  const controller = new AbortController();
  const end = () => controller.abort();
  const keypress = (_, key) => { if (key?.ctrl && key.name === 'd') end(); };
  input.once('end', end);
  input.on('keypress', keypress);
  try {
    if (input.readableEnded) end();
    return await select(config, { input, output, signal: controller.signal });
  } finally {
    input.off('end', end);
...
export async function resolveInitOptions(options, { detection, env = process.env,
  input = process.stdin, output = process.stderr, select = selectInitOption } = {}) {
  ...
  let provider = options.provider || (options.framework !== 'auto' && options.framework) || report.selected;
  ...
  if (!['openspec', 'speckit'].includes(provider)) throw new Error('provider_selection_required: choose openspec or speckit');
