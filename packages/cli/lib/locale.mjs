...
//#region packages/cli/src/lib/locale.mts
const catalogs = /* @__PURE__ */ new Map();
const supported = /* @__PURE__ */ new Set(["en", "zh-CN"]);
const languageContext = new AsyncLocalStorage();
const withLocale = (locale, action) => languageContext.run(locale, action);
function systemLanguages(platform = process.platform, run = spawnSync) {
	const command = platform === "darwin" ? ["/usr/bin/defaults", [
		"read",
		"-g",
		"AppleLanguages"
		...
		"-NonInteractive",
		"-Command",
		"[Console]::OutputEncoding=[System.Text.Encoding]::UTF8;(Get-UICulture).Name"
	]] : null;
	if (!command) return [];
	try {
...
function detectLocale({ explicit, env = process.env, intl = true, system = systemLanguages } = {}) {
	if (explicit === void 0 && env === process.env && languageContext.getStore()) return languageContext.getStore();
	if (explicit !== void 0) return normalizeLocale(explicit) || "en";
	for (const value of [
		env.SPEC_AUTONOMOUS_LANG,
		env.LC_ALL,
		env.LC_MESSAGES,
		env.LANGUAGE
	]) {
	...
	if (intl) try {
		return normalizeLocale(new Intl.DateTimeFormat().resolvedOptions().locale) || "en";
	} catch {}
	return "en";
}
function catalog(locale = detectLocale()) {
...
function message(key, locale = detectLocale(), variables = {}) {
...
function localizeError(error, locale = detectLocale()) {
...
//#endregion
export { catalog, detectLocale, languageArg, localizeError, message, normalizeLocale, systemLanguages, withLocale };
