import { errorCode, errorMessage } from "./errors.mjs";
import { message } from "./locale.mjs";
import { platformFor } from "./platform.mjs";
import { findExecutable, nodeCommand, npmArgv, runProcess } from "./provider-process.mjs";
import { uvHashes, versions } from "./provider-versions.mjs";
...
//#region packages/cli/src/lib/providers.mts
const providerNames = ["openspec", "speckit"];
function providerName(value) {
	if (!providerNames.includes(value)) throw new Error("provider_selection_required: choose openspec or speckit");
	return value;
}
function providerHome(env = process.env) {
	const base = process.platform === "win32" ? env.LOCALAPPDATA || join(homedir(), "AppData/Local") : process.platform === "darwin" ? join(homedir(), "Library/Application Support") : env.XDG_DATA_HOME || join(homedir(), ".local/share");
...
async function withInstallLock(directory, action, { timeout = 31e4 } = {}) {
	...
	while (true) try {
		...
		let owner;
		try {
			owner = JSON.parse(await readFile(join(directory, "owner.json"), "utf8"));
		} catch {}
		const observed = await stat(directory).catch(() => null);
		const age = Date.now() - (observed?.mtimeMs ?? Date.now());
		if (owner && !alive(owner.pid) && !alive(owner.child) || !owner && age > 3e4) {
			...
			try {
				...
				let latest;
				try {
					latest = JSON.parse(await readFile(join(directory, "owner.json"), "utf8"));
				} catch {}
				const current = await stat(directory);
				if (latest && !alive(latest.pid) && !alive(latest.child) || !latest && age > 3e4 && current.ino === observed?.ino && current.dev === observed?.dev) await rm(directory, {
					recursive: true,
					force: true
				});
				else await rm(reaper, {
					recursive: true,
					force: true
				});
			} catch (e) {
				...
				if (code === "EEXIST") {
					let reaperOwner;
					try {
						reaperOwner = JSON.parse(await readFile(join(reaper, "owner.json"), "utf8"));
					} catch {}
					const reaperAge = Date.now() - (await stat(reaper).catch(() => ({ mtimeMs: Date.now() }))).mtimeMs;
					if (reaperOwner && !alive(reaperOwner.pid) || !reaperOwner && reaperAge > 3e4) await rm(reaper, {
						recursive: true,
						force: true
					});
				}
...
async function downloadVerified(url, destination, expected, fetcher = fetch) {
	...
	for await (const chunk of response.body) {
		size += chunk.length;
		if (size > 134217728) throw new Error("provider_download_too_large");
		chunks.push(chunk);
	}
...
function createProviderManager({ env = process.env, home = providerHome(env), run = runProcess, find = (name) => findExecutable(name, env), platform = platformFor, download = downloadVerified, log = (text) => {
	process.stderr.write(`[providers] ${text}\n`);
} } = {}) {
	const childEnv = {
		...env,
	...
	const commandFor = (name, dir) => name === "openspec" ? [nodeCommand(env), join(dir, "node_modules/@fission-ai/openspec/bin/openspec.js")] : [join(dir, "bin", process.platform === "win32" ? "specify.exe" : "specify")];
	async function probe(name, command, root) {
		try {
			const result = await run([...command, name === "speckit" ? "version" : "--version"], {
				cwd: root,
				env: childEnv,
				timeout: 15e3
			});
			const match = `${result.stdout}\n${result.stderr}`.match(/\b(\d+\.\d+\.\d+(?:[-+.][\w.]+)?)\b/);
			return result.code === 0 && match ? {
				ready: true,
				version: match[1]
			} : {
				ready: false,
	...
	function external(name, root) {
		...
		if (executable.endsWith(".cmd")) {
			...
			if (name === "openspec" && existsSync(entry)) return {
				command: [nodeCommand(env), entry],
				source: "path"
			};
		...
		return {
			command: name === "openspec" && /\.[cm]?js$/.test(file) ? [nodeCommand(env), file] : [executable],
			source: "path"
		};
	}
	async function managed(name) {
		try {
			...
			if (["ENOENT", "ENOTDIR"].includes(errorCode(error) ?? "") || error instanceof SyntaxError) return null;
			throw error;
		}
	}
	async function status(name, { root = process.cwd(), managed: forceManaged = false, command } = {}) {
		...
		if (!forceManaged) {
			const existing = command ? {
				command,
				source: "configured"
			} : external(name, resolve(root));
			if (existing) return {
				provider: name,
				...existing,
				...await probe(name, existing.command, root)
			};
		}
		return await managed(name) || {
			provider: name,
			ready: false,
			source: "missing",
	...
	async function installUv(onChild) {
		...
		if (existsSync(executable)) {
			const check = await run([executable, "--version"], {
				env: childEnv,
				timeout: 15e3
			});
		...
		try {
			...
			await chmod(extracted, 493);
			const check = await run([extracted, "--version"], {
				env: childEnv,
				timeout: 15e3
			});
	...
	async function ensure(name, options = {}) {
		...
		return withInstallLock(join(home, ".install-lock"), async (onChild) => {
			...
			await mkdir(directory, { recursive: true });
			log(message("log.install", void 0, {
				provider: name,
				version: versions[name],
				directory
			}));
			if (name === "openspec") {
				...
				await checked(process.versions.bun ? [
					process.execPath,
					"add",
					"--ignore-scripts",
					"--exact",
					`@fission-ai/openspec@${versions.openspec}`
				] : [
					...npmArgv(env),
					"install",
					"--ignore-scripts",
				...
				let uv = find("uv");
				if (uv) {
					if ((await run([uv, "--version"], {
						env: childEnv,
						timeout: 15e3
					})).code !== 0) throw new Error("provider_uv_invalid: existing uv failed validation");
				...
				await checked([
					...
					"--python",
					">=3.11",
					`specify-cli==${versions.speckit}`
				], {
					cwd: directory,
					onChild,
					env: {
						...childEnv,
						UV_TOOL_DIR: join(directory, "tools"),
						UV_TOOL_BIN_DIR: join(directory, "bin"),
...
//#endregion
export { createProviderManager, downloadVerified, providerHome, providerName, providerNames, withInstallLock };
