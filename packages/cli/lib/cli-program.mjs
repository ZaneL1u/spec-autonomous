...
import { bridgeEnvironment, cliSource, createProviderContext, initializeProvider, nativeAction, needsProvider, providerOperation } from "./provider-cli.mjs";
import { serveProviderMcp } from "./provider-mcp.mjs";
import { resolveInitOptions } from "./init-options.mjs";
import { Argument, Command, CommanderError, Option } from "commander";
//#region packages/cli/src/lib/cli-program.mts
function syntax(message) {
	return new CommanderError(2, "invalid_arguments", message);
}
function outputPreferences(argv) {
	const parser = new Command().helpOption(false).allowUnknownOption().allowExcessArguments().option("--json").option("--format <format>").exitOverride().configureOutput({ writeErr: () => {} });
	try {
...
function selectedOptions(command) {
	const selected = {};
	for (let current = command; current; current = current.parent) for (const [key, value] of Object.entries(current.opts())) {
...
function addDefinition(command, definition, locale) {
	...
	const flags = [definition.short && `-${definition.short}`, definition.long && `--${definition.long}`].filter(Boolean).join(", ") + (definition.takes_value ? ` <${definition.id}>` : "");
...
function createProgram(schema, { native, providers, locale = detectLocale(), stdout = (text) => {
	process.stdout.write(text);
}, stderr = (text) => {
	process.stderr.write(text);
} }) {
	function configure(command) {
		return command.enablePositionalOptions().exitOverride().helpOption("-h, --help", message("option.help", locale)).configureOutput({
			writeOut: stdout,
			writeErr: stderr
	...
	const program = configure(new Command(schema.name)).description(schema.description).version(schema.version, "-V, --version", message("option.version", locale));
	...
	function register(parent, definition) {
		...
		for (const arg of definition.arguments) addDefinition(command, definition.name === "init" && arg.id === "agent" ? {
			...arg,
		...
		command.action(function() {
			return native(this, selectedOptions(this));
		});
	...
	const globals = schema.arguments.filter((arg) => [
		"path",
		"framework",
		"json",
		"format",
	...
	const providerGlobals = (command) => {
		for (const arg of globals) addDefinition(command, arg.id === "format" ? {
			...arg,
			choices: ["json"]
		} : arg, locale);
		return command;
	...
	for (const name of [
		...
		const command = providerGlobals(configure(new Command(name))).description({
			status: message("command.providers_status", locale),
			ensure: message("command.providers_ensure", locale),
			exec: message("command.providers_exec", locale)
		}[name]).addArgument(new Argument(name === "exec" ? "<provider>" : "[provider]", message("help.provider", locale)).choices(["openspec", "speckit"]));
	...
	providerGroup.action(function() {
		return providers("status", selectedOptions(this));
	});
	program.addCommand(providerGroup);
	program.addCommand(configure(new Command("help")).description(message("command.help", locale)).argument("[commands...]", message("help.command_path", locale)).action((names) => {
...
function initArguments(schema, options) {
	...
	for (const definition of [...schema.arguments, ...init.arguments]) {
		...
		const value = options[new Option(`--${definition.long}`).attributeName()];
		if (value !== void 0 && value !== false) argv.push(`--${definition.long}`, ...definition.takes_value ? [String(value)] : []);
	}
	if (options.provider) {
		...
		if (!options.framework || options.framework === "auto") return initArguments(schema, {
			...options,
			provider: void 0,
			framework: options.provider
		});
	}
	return argv;
}
async function executeCli(argv, locale, { binary: explicitBinary, run = runProcess, forward = forwardProcess, context = createProviderContext, mcp = serveProviderMcp, initialize = initializeProvider, initOptions = resolveInitOptions, input = process.stdin, promptOutput = process.stderr, stdout = (text) => {
	process.stdout.write(text);
}, stderr = (text) => {
	process.stderr.write(text);
} } = {}) {
	...
	try {
		...
		const program = createProgram(schema, {
			locale,
			stdout,
			stderr: (text) => {
				diagnostic += text;
			...
			native: async (command, options) => {
				...
				} else if (parsed.command.name === "mcp") {
					exitCode = await mcp(ctx);
					return;
				...
				exitCode = await forward([binary, ...args], { env: {
					...bridgeEnvironment(),
					SPEC_AUTONOMOUS_LANG: locale
				} });
			},
			providers: async (operation, options, provider, nativeArgs = []) => {
				for (const key of Object.keys(options)) if (![
					...
					"lang",
					"managed"
				].includes(key)) throw syntax(`option '${key}' is only available for native capabilities`);
				...
				const ctx = context(binary, {
					...options,
					lang: locale
				});
				if (operation === "exec") exitCode = await forward([...(await ctx.ensureSelected(provider, {
					allowMissing: true,
					...
					const data = await providerOperation(ctx, {
						operation,
						provider,
						managed: Boolean(options.managed)
					});
		...
		if (preferences.json || preferences.format === "json") stdout(JSON.stringify({
			schema_version: 1,
			error: {
				code,
				message: localized
...
//#endregion
export { createProgram, outputPreferences, runCli };
