...
复现：`node --test packages/cli/test/init-options.test.mjs tests/e2e/init.test.mjs`；真实工具验收使用 `node scripts/test-init-real.mjs --run`。日志与真实测试仓库路径记录于本机 `.artifacts/interactive-init-*`。
