...
export function createMock(output,framework='openspec',{goalOnly=false,failOnce='',sleepMs=80,phaseRepair=false,milestoneRepair=false}={}){
  ...
  const write=(path,body)=>{mkdirSync(dirname(join(output,path)),{recursive:true});writeFileSync(join(output,path),body);};
  const run=(args)=>{const r=spawnSync('git',args,{cwd:output,encoding:'utf8'});if(r.status!==0)throw new Error(r.stderr);};
  const node=process.execPath;
  const check=(...tests)=>({argv:[node,'--test',...tests],cwd:'.'});
  const add={name:'add',path:'src/add.mjs',test:'tests/add.test.mjs',content:'export const add = (a, b) => a + b;\n',broken:'export const add = (a, b) => a - b;\n'};
  const multiply={name:'multiply',path:'src/multiply.mjs',test:'tests/multiply.test.mjs',content:'export const multiply = (a, b) => a * b;\n',broken:'export const multiply = (a, b) => a + b;\n'};
  const service={name:'service',path:'src/service.mjs',test:'tests/service.test.mjs',reads:['src/add.mjs','src/multiply.mjs'],content:"import { add } from './add.mjs';\nimport { multiply } from './multiply.mjs';\nexport const calculate = (a, b) => ({sum:add(a,b),product:multiply(a,b)});\n",broken:'export const calculate = () => ({});\n'};
  const repair={name:'version',path:'src/version.mjs',test:'checks/version.mjs',content:'export const version = 1;\n',broken:'export const version = 0;\n'};
  const phases=[{id:'P001',label:'1',slug:'arithmetic',title:'Arithmetic primitives',depends_on:[],verification:[check(add.test,multiply.test)],tasks:[add,multiply]},{id:'P002',label:'2',slug:'calculator',title:'Calculator service',depends_on:['P001'],verification:[check(service.test)],tasks:[service]}];
  ...
  for(const [task,body] of [[add,"assert.equal(add(2,3),5); assert.equal(add(-4,3),-1);"],[multiply,"assert.equal(multiply(2,3),6); assert.equal(multiply(-4,3),-12);"],[service,"assert.deepEqual(calculate(2,3),{sum:5,product:6});"]]){
  ...
  const runner=join(workspace,'tests/mock-agent.mjs');
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  console.log(JSON.stringify(createMock(output,values.framework,{goalOnly:values['goal-only'],failOnce:values['fail-once']}),null,2));
}
