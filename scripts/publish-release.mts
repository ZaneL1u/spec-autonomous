...
import { errorMessage } from '../packages/cli/lib/errors.mjs';

const REGISTRY = 'https://registry.npmjs.org/';
const MAX_ARCHIVE = 128 * 1024 * 1024;
type DigestEncoding='hex'|'base64';
interface TarEntry {bytes:Buffer;mode:number;directory:boolean}
interface ReleaseManifest {name:string;version:string;private?:boolean;scripts?:Record<string,unknown>;peerDependencies?:Record<string,unknown>;bundledDependencies?:Record<string,unknown>;bundleDependencies?:Record<string,unknown>;dependencies?:Record<string,string>;optionalDependencies?:Record<string,string>;publishConfig?:{access?:string;registry?:string};repository?:string|{url?:string};os?:string[];cpu?:string[];libc?:string[];bin?:Record<string,string>}
interface ReleasePackage {name:string;version:string;filename:string;path:string;sha512:string;sha256:string|undefined;bytes:Buffer;manifest:ReleaseManifest;action?:'reused'|'published'}
interface NpmResult {status:number|null;stdout:string;stderr?:string;signal?:NodeJS.Signals|null}
type NpmAdapter=(args:string[],options:{cwd:string})=>Promise<NpmResult>;
interface PublishOptions {directory?:string;publish?:boolean;tag?:string;visibilityAttempts?:number}
interface PublishAdapters {npm?:NpmAdapter;wait?:(ms:number)=>Promise<void>}
const digest = (bytes:NodeJS.ArrayBufferView, algorithm:string, encoding:DigestEncoding = 'hex') => createHash(algorithm).update(bytes).digest(encoding);
const integrity = (bytes:NodeJS.ArrayBufferView) => `sha512-${digest(bytes, 'sha512', 'base64')}`;
const reject = (message:string):never => { throw new Error(`release_rejected: ${message}`); };
const nonempty = (value:unknown) => typeof value === 'object' && value !== null && Object.keys(value).length > 0;
...
export function tarEntries(compressed:NodeJS.ArrayBufferView):Map<string,TarEntry> {
  const tar = gunzipSync(compressed, { maxOutputLength: MAX_ARCHIVE });
  const entries = new Map<string,TarEntry>();
  const string = (field:Buffer) => field.toString('utf8').replace(/\0.*$/s, '');
  ...
  return reject('unterminated tar archive');
}
...
function nativeHeader(bytes:Buffer, platform:Platform) {
...
export function inspectRelease(directory:string) {
  ...
  for (const filename of files) {
    ...
    const manifest = JSON.parse(manifestEntry!.bytes.toString('utf8')) as ReleaseManifest;
  ...
  return ordered as ReleasePackage[];
}

export async function defaultNpm(args:string[], options:{cwd:string}) {
...
export async function publishRelease({ directory = '.artifacts/npm', publish = false, tag = 'next', visibilityAttempts = 6 }:PublishOptions = {}, { npm = defaultNpm, wait = ms => new Promise(resolve => setTimeout(resolve, ms)) }:PublishAdapters = {}) {
  ...
  const repository = (p:ReleasePackage|undefined) => {
  ...
  const report:{mode:string;registry:string;tag:string;version:string;repository:string|null;publication_prerequisites:string[];native_validation:string;packages:Array<{name:string;version:string;filename:string;sha256:string|undefined;sha512:string;action?:string}>} = { mode: publish ? 'publish' : 'dry-run', registry: REGISTRY, tag, version: packages[0]!.version, repository: repo ?? null, publication_prerequisites: prerequisites, native_validation: 'format-and-architecture only; execution evidence belongs to the successful release-artifacts workflow', packages: packages.map(({ name, version, filename, sha256, sha512 }) => ({ name, version, filename, sha256, sha512 })) };
  ...
  try {
    ...
    const invoke = (args:string[]) => npm(args, { cwd: stage });
    const visible = async (p:ReleasePackage) => {
    ...
    const waitVisible = async (p:ReleasePackage) => {
    ...
    const ensure = async (p:ReleasePackage) => {
