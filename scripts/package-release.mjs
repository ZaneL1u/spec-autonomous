...
export function assemble(input, output) {
  ...
  for (const p of platforms) {
    ...
    const native = { name, version: manifest.version, description: `Native binary for spec-autonomous (${p.key})`, license: manifest.license, os: [p.os], cpu: [p.cpu], ...(p.libc ? { libc: [p.libc] } : {}), files: ['bin', 'LICENSE'], publishConfig: { access: 'public' } };
  ...
  mkdirSync(wrapper);
  for (const name of ['bin', 'lib', 'README.md', 'LICENSE']) cpSync(join(source, name), join(wrapper, name), { recursive: true });
  chmodSync(join(wrapper, 'bin/spec-autonomous.mjs'), 0o755);
  writeFileSync(join(wrapper, 'package.json'), JSON.stringify({ ...manifest, files: ['bin', 'lib', 'README.md', 'LICENSE'], optionalDependencies }, null, 2) + '\n');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { values } = parseArgs({ options: { input: { type: 'string', default: '.artifacts/binaries' }, output: { type: 'string', default: '.artifacts/release' } } });
  console.log(JSON.stringify(assemble(resolve(values.input), resolve(values.output)), null, 2));
}
