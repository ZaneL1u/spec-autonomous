...
const env:NodeJS.ProcessEnv = { ...process.env, SPEC_AUTONOMOUS_PROVIDER_HOME: home, UV_PYTHON_PREFERENCE: 'only-managed' };
...
for (const provider of ['openspec', 'speckit'] as const) {
  ...
  results.push({ provider, project, ready, detection: detect, binding: JSON.parse(bind.stdout) as unknown });
}
