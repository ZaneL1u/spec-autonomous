...
fn cards(run: &Run, p: &Phase, snapshot: &crate::model::Snapshot) -> Value {
    ...
    if !has_checks {
        out.push(json!({"id":key("verification"),"question":"Which verification contract should this phase use?","recommended":"explicit-test","alternatives":[{"id":"explicit-test","label":"Choose an explicit test command","description":"Use a concrete test file or supported discovery command."},{"id":"native-checks","label":"Use native/project checks","description":"Keep checks supplied by the existing framework or project configuration."}],"why":"No executable acceptance gate is currently specified for this phase.","impact":"The choice becomes the phase verification input for planning.","requires_user":true,"status":"unresolved"}));
    }
    let has_parallel = snapshot.tasks.iter().any(|t| !t.done && t.parallel);
    if has_parallel {
        out.push(json!({"id":key("ordering"),"question":"Can explicitly parallel native tasks run together?","recommended":"respect-native-order","alternatives":[{"id":"respect-native-order","label":"Respect native order","description":"Keep conservative order unless the native task metadata marks siblings parallel."},{"id":"parallel-siblings","label":"Parallelize marked siblings","description":"Allow only explicit [P] siblings in the same phase to run together."}],"why":"The native task graph marks parallel candidates but ordering is still a project choice.","impact":"Changes scheduling only; dependency and write-set checks still apply.","requires_user":true,"status":"unresolved"}));
    }
