...
impl Coordinator<'_> {
    pub(super) fn build_plan(&mut self, phase: &Phase, snapshot: &Snapshot) -> Result<Plan> {
        ...
        for (batch, rows) in pending.chunks(limit).enumerate() {
            ...
            partial.metadata["verification_environment"] =
                crate::verification_preflight::environment_with_overrides(
                    &self.project(),
                    &self.run.config.runner.environment,
                );
            let task_id = format!("plan-{}-{}", phase.id, batch + 1);
            if let Some(failed) = self.run.attempts.iter().rev().find(|a| {
                a.kind == "plan-tasks"
                    && a.task_id == task_id
                    && a.phase_id == phase.id
                    && a.status == "failed"
            }) {
                partial.metadata["previous_planning_failure"] = serde_json::json!({
                    "request_id":failed.id,"error":failed.error,
                    "action":"Correct the rejected plan before returning a new candidate; preserve native task coverage."
                });
            }
            let instruction = format!(
                "Return an execution Plan for phase {}. Use schema_version=1 and exact snapshot.source_hash. Cover EVERY source task in snapshot.tasks using its exact ID; do not add other sources even when the native tracking file contains more rows. Tasks contain id,description,source_ids,depends_on,reads,writes,verification. Preserve native order and phase barriers; only independent [P] siblings may parallelize in Spec Kit. Every task must have meaningful, individually runnable verification argv unless project-level checks are configured. Do not edit files. This is planning batch {}; dependency references must be internal to this batch; the host adds conservative cross-batch barriers. {}",
                phase.id,
                batch + 1,
            ...
            let mut plan = match candidate {
                Ok(plan) => plan,
                Err(error) => {
                    let attempt = &mut self.run.attempts[index];
                    attempt.status = "failed".into();
                    attempt.error = Some(format!("{error:#}"));
                    attempt.finished_at = Some(paths::now());
                    self.save("planning_batch_rejected")?;
                    bail!(
                        "needs_input: planning candidate rejected; prepare the run again for a corrected plan: {error:#}"
                    );
                }
            };
            if batched {
