...
import {fixture,cli,git,join,workspace} from './helpers.mjs';
import {raw,cleanEnv} from '../mock-host.mjs';
for(const point of ['before_receipt_file','after_receipt_file'])test(`receipt restart reconciles ${point} without another semantic worker`,{timeout:30000},t=>{
 ...
 const saved=raw(root,['status',first.data.id]).data;assert.equal(saved.attempts.find(a=>a.id===request.request_id).status,'receiving');
 ...
 const status=raw(root,['status',first.data.id]).data;assert.equal(status.attempts.filter(a=>a.task_id===request.task_id).length,1);assert.equal(status.attempts.find(a=>a.id===request.request_id).status,'accepted');
});
