...
test('run.cleanup CLI previews terminal resources, rejects changed state and applies a reviewed cleanup',{timeout:120000},t=>{
  ...
  const call=args=>cli(root,['tools','call','run.cleanup','--input',JSON.stringify({...args,run_id:done.data.id,view:'full'})]);
  ...
  assert.ok(preview.data.worktrees.every(w=>existsSync(w.path)));
  ...
  assert.ok(fresh.data.retained_details.some(r=>r.path===dirty.path&&r.reason==='dirty'));
  ...
  assert.ok(applied.data.removed.every(path=>!existsSync(path)));
