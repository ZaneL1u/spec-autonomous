...
import {fixture,cli,start,until,git,configure,join} from './helpers.mts';
...
test('OpenSpec: native planning, parallel fresh workers, repair and full verified delivery', {timeout:120000}, t=>{
  ...
  assert.equal(new Set(attempts.map((a: any)=>a.id)).size,attempts.length);
  assert.ok(attempts.some((a: any)=>a.kind==='implement'&&a.status==='failed'));
  const writes=attempts.filter((a: any)=>a.kind==='implement');
  assert.equal(new Set(writes.map((a: any)=>a.worktree)).size,writes.length);
  assert.ok(writes.some((a: any,i: any)=>writes.some((b: any,j: any)=>i!==j&&Date.parse(a.started_at)<Date.parse(b.finished_at)&&Date.parse(b.started_at)<Date.parse(a.finished_at))));
  for(const p of ['arithmetic','calculator'])assert.ok(!readFileSync(join(root,`openspec/changes/${p}/tasks.md`),'utf8').includes('- [ ]'));
  assert.ok(r.data.evidence.every((e: any)=>e.revision&&e.log_hash&&existsSync(e.log)));
  const report=cli(root,['report',r.data.id]);assert.equal(report.status,0,report.details);assert.ok(report.data.events.some((e: any)=>e.kind==='task_retry'));
});
...
for(const framework of ['openspec','speckit'])test(`${framework}: a goal creates native artifacts and roadmap before execution`, {timeout:120000}, t=>{
  ...
  assert.ok(r.data.attempts.some((a: any)=>a.kind==='native-planning'));
});
...
for(const point of ['after_candidate_patch','after_source_writeback','before_intent_finalize','after_candidate_commit','after_git_advance','after_origin_advance'])test(`crash recovery reconciles ${point} without duplicating accepted work`,{timeout:150000},t=>{
  ...
  const accepted=result.data.intents.filter((i: any)=>i.state==='accepted');
  assert.equal(new Set(accepted.map((i: any)=>i.task_id)).size,accepted.length);
  assert.equal(git(root,['status','--porcelain']),'');
});
...
test('progress observes concurrent workers from a linked worktree and pause stops them',{timeout:120000},async t=>{
  ...
  const worker=progress.worktrees.find((w: any)=>w.kind==='managed-worker'&&w.status==='claimed');
  assert.ok(worker);
