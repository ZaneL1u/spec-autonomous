...
import {fixture, edit, git, join, workspace} from './helpers.mts';
import {raw, drive, cleanEnv, resolveDiscussion} from '../mock-host.mts';

function execute(root: any, request: any, changeResult:(result:any)=>void = () => {}) {
  ...
  const worker = spawnSync(process.execPath, [join(workspace, 'tests/mock-agent.mts')], {
    cwd: request.project,
...
test('a failed verification command can be revised and delivered in the same run without redoing accepted work', {timeout: 120000}, async t => {
  ...
  for (let round = 0; round < 16 && current.data?.work?.[0]?.kind !== 'implement'; round++) {
    ...
    current = execute(root, request, (result: any) => {
  ...
  const failedAttempt = stopped.data.attempts.find((a: any) => a.id === failedRequest.request_id);
  assert.equal(failedAttempt.status, 'failed');
  assert.match(failedAttempt.error, /verification_failed/);
  const failure = stopped.data.evidence.find((e: any) => e.exit_code !== 0 && e.argv.includes(badCheck.argv[2]));
  ...
  const acceptedAttempts = stopped.data.attempts.filter((a: any) => a.kind === 'implement' && a.task_id === acceptedId).map((a: any) => a.id);
  ...
  const call = (args: any) => raw(root, ['tools', 'call', 'run.revise', '--input', JSON.stringify(args), '--view', 'full']);
  ...
  assert.deepEqual(delivered.data.attempts.filter((a: any) => a.kind === 'implement' && a.task_id === acceptedId).map((a: any) => a.id), acceptedAttempts);
  assert.ok(delivered.data.attempts.some((a: any) => a.id === failedRequest.request_id && a.status === 'failed'));
  assert.ok(delivered.data.attempts.some((a: any) => a.task_id === targetId && a.status === 'integrated'));
  assert.ok(delivered.data.evidence.some((e: any) => e.log === failure.log && e.exit_code !== 0));
